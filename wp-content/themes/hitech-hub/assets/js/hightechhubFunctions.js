...
jQuery(document).ready(function() {
            ...
            var popupClosed = false; // Flag to track if the popup has been closed

            function showPopup() {
                if (!popupClosed) {
                    jQuery('#aboutproject').show();
					jQuery('#aboutproject').removeClass('fade');
                }
            }

            function startInactivityTimer() {
                clearTimeout(inactivityTimeout);
                inactivityTimeout = setTimeout(showPopup, 15000);
            }
            ...
            window.ontouchstart = resetInactivityTimer; // catches touchscreen swipes
            window.ontouchmove = resetInactivityTimer;  // required by some devices
            window.onclick = resetInactivityTimer;      // catches touchpad clicks
            window.onkeypress = resetInactivityTimer;   // catches keyboard activity

            jQuery('.close-popup').click(function() {
                jQuery('#aboutproject').hide();
