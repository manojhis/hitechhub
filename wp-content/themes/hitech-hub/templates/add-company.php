			...
			<div class="col-sm-12 col-12">
				<div class="contactus-card">
					<form enctype="multipart/form-data" id="company_register" method="POST" name="company_register" class="add-company-page-form form" novalidate="novalidate">
						...
						<input type="hidden" name="wd_status" value="1">
						<div class="input-box">
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" id="company_website" name="company_website" placeholder="https://www.website.xyz" required="required" >
							</div>
							<div class="form-group">
								<label>Country Name (Max.3)*</label>
								<input type="text" class="form-control valid" id="company_location" name="company_location" placeholder="USA, Ukraine, Australia" aria-invalid="false" required="required" >
							</div>
							<div class="form-group">
								<label>Company Established On*</label>
								<select name="company_established_date" class="form-control required selected-opetion" id="company_established_date" required="required" >
							...
							<div class="form-group">
								...
								<select name="company_employees" class="form-control required selected-opetion" id="company_employees" required="required" >
							...
							<div class="form-group">
								...
								<select name="company_price" class="form-control required selected-opetion" id="company_price" required="required" >
							...
							<div class="form-group">
								...
								<input type="text" class="form-control" id="email" name="email" placeholder="email@website.xyz" required="required" >
							</div>		
							<div class="form-group">
								<label>Company Contact*</label>
								<input type="text" class="form-control numberOnly" id="company_contact" name="company_contact"  placeholder="+1-111-111-1111" required="required" >
							</div>
							<div class="form-group">
							...
							<div class="form-group half-row mT20">
								...
								<textarea name="company_services" id="company_services" maxlength="300" class="form-control" rows="3" required="required" ></textarea>
							</div>
							<div class="form-group half-row">
								<label>Key Client*</label>
								<textarea name="company_key_client" id="company_key_client" maxlength="150" class="form-control" rows="3"required="required" ></textarea>
							</div>
							<div class="form-group full-row">
								<label>Description*</label>
								<textarea name="company_description" id="company_description" class="form-control required" maxlength="700" rows="3" required="required" ></textarea>
							</div>
...
jQuery(document).ready(function(e){
	jQuery(document).on('click', '#company-submit-form', function(e){
		...
		checkForEmpty( "company_description" );
		checkForEmpty( "company_contact" );
		checkForEmpty( "company_key_client" );
		checkForEmpty( "company_services" );
		checkForEmpty( "email" );
		checkForEmpty( "company_price" );
		checkForEmpty( "company_employees" );
		checkForEmpty( "company_established_date" );
		checkForEmpty( "company_location" );
		checkForEmpty( "company_website" );
		checkForEmpty( "company_title" );

		function checkForEmpty( filedName ){
			var val = jQuery('#'+filedName).val();
			if(val == ''){
				jQuery('#'+filedName).css('outline', '2px solid red');
				jQuery('#error_'+filedName).remove();
				if( jQuery("#"+filedName).prop('tagName') == "SELECT" ){
					jQuery('#'+filedName).next().after('<span class="error" id="error_'+filedName+'" style="color:red; display:block;">Please fill out this field.</span>');
				}else{
					jQuery('#'+filedName).after('<span class="error" id="error_'+filedName+'" style="color:red; display:block;">Please fill out this field.</span>');
				}
				jQuery('#'+filedName).focus();
				err++;
				return false;
			}else{
				jQuery('#'+filedName).css('outline', 'none');
				jQuery('#error_'+filedName).remove();
			}
		}
		...
		if(companyLocationValue != '' && company_values.length > 3) {
			jQuery('#company_location').css('outline', '2px solid red');
			jQuery('#error_company_location').remove();
			jQuery('#company_location').after('<span class="error" id="error_company_location" style="color:red; display:block;">You cannont choose more than three countries.</span>');
			jQuery('#company_location').focus();
			err++;
            return false;
		}
		if(company_captcha_ans == ''){
			jQuery('#company_captcha_ans').css('outline', '2px solid red');
			jQuery('#error_company_captcha_ans').remove();
			jQuery('.input-field').after('<span class="error" id="error_company_captcha_ans" style="color:red; display:block;">Please fill out this field.</span>');
			...
			jQuery('#company_captcha_ans').css('outline', '2px solid red');
			jQuery('#error_company_captcha_ans').remove();
			jQuery('.input-field').after('<span class="error" id="error_company_captcha_ans" style="color:red; display:block;">Captcha is Invalid!</span>');
			jQuery('#company_captcha_ans').focus();
			err++;
            return false;
		} else{
		...
		if(err == 0){
			...
			jQuery('button#company-submit-form').html('Please wait...');
			jQuery('button#company-submit-form').prop('disabled', true);
			jQuery.ajax({
				...
				jQuery('button#company-submit-form').html('Submit');
				jQuery('button#company-submit-form').prop('disabled', false);
				},
				error			: function(a){
					console.log(a);
				}
